...
// RED team RED flag Capture
"if 29 "
	"yb -102 "
	"xr -105 "
	"pic 29 "
"endif "
// RED team BLUE flag Capture
"if 16 "
	"yb -102 "
	"xr -80 "
	"pic 16 "
"endif "
// BLUE team RED flag Capture
"if 30 "
	"yb -75 "
	"xr -105 "
	"pic 30 "
"endif "
// BLUE team BLUE flag Capture
"if 31 " 
	"yb -75 "
	"xr -80 "
	"pic 31 "
...
static void CTFFlagThink(edict_t *ent)
{
	...
	if(!(ctfgame.flag1 == 1 && ctfgame.flag2 == 1) && !(ctfgame.flag1 == 2 && ctfgame.flag2 == 2))
	{
		ctfgame.flagtimer = -1;
	}

	if(ctfgame.flag1 == 1 && ctfgame.flag2 == 1)
	{
		if(ctfgame.flagtimer <= -1)
			ctfgame.flagtimer = 15;
		ctfgame.flagtimer -= .1;
		...
		for (i = 1; i <= maxclients->value; i++) 
		{
			...
			if (!player->inuse)
				continue;
			gi.centerprintf(player, "%.0f..", ctfgame.flagtimer);	
		}
		if(ctfgame.flagtimer <= 0 && ctfgame.flagtimer > -1)
		{
			ctfgame.flagtimer = -1;
			ctfgame.flag1 = 0;
			ctfgame.flag2 = 0;
			ctfgame.last_flag_capture = level.time;
			ctfgame.last_capture_team = 1;
			ctfgame.team1++;
			for (i = 1; i <= maxclients->value; i++) 
			{
				player = &g_edicts[i];
				if (!player->inuse)
					continue;
				gi.centerprintf(player, "Red team scores");	
			}
		}

	}
	if(ctfgame.flag1 == 2 && ctfgame.flag2 == 2)
	{
		if(ctfgame.flagtimer <= -1)
			ctfgame.flagtimer = 15;
		ctfgame.flagtimer -= .1;
		...
		if(ctfgame.flagtimer <= 0 && ctfgame.flagtimer > -1)
		{
			...
			for (i = 1; i <= maxclients->value; i++) 
			{
				...
				if (!player->inuse)
					continue;
				gi.centerprintf(player, "Blue team scores");	
			}
		}
	}
...
void Touch_Flag (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	edict_t *e;
	...
	if (strcmp(ent->classname, "item_flag_team1") == 0)
	{
		ctfgame.flag1 = other->client->resp.ctf_team;
	}
	if (strcmp(ent->classname, "item_flag_team2") == 0)
	{
		ctfgame.flag2 = other->client->resp.ctf_team;
	}
}
...
void SetCTFStats(edict_t *ent)
{
	...
	//Red team == 1, Blue team == 2
	if((ctfgame.flag1 == 1 && ctfgame.flag2 == 1) || (ctfgame.flag1 == 2 && ctfgame.flag2 == 2))
		ent->client->ps.stats[STAT_TIMER_FLAG] = ctfgame.flagtimer;
	else
		ent->client->ps.stats[STAT_TIMER_FLAG] = 0;
	if(ctfgame.flag1 == 1)
		ent->client->ps.stats[STAT_RED_CAPTURE_RED] = gi.imageindex ("i_ctf1t");
	else
		ent->client->ps.stats[STAT_RED_CAPTURE_RED] = 0;
	if(ctfgame.flag2 == 1)
		ent->client->ps.stats[STAT_RED_CAPTURE_BLUE] = gi.imageindex ("i_ctf2t");
	else
		ent->client->ps.stats[STAT_RED_CAPTURE_BLUE] = 0;
	if(ctfgame.flag1 == 2)
		ent->client->ps.stats[STAT_BLUE_CAPTURE_RED] = gi.imageindex ("i_ctf1t");
	else
		ent->client->ps.stats[STAT_BLUE_CAPTURE_RED] = 0;
	if(ctfgame.flag2 == 2)
		ent->client->ps.stats[STAT_BLUE_CAPTURE_BLUE] = gi.imageindex ("i_ctf2t");
	else
		ent->client->ps.stats[STAT_BLUE_CAPTURE_BLUE] = 0;
...
void CTFGrapplePull(edict_t *self)
{
	...
	if (self->owner->client->ctf_grapplestate > CTF_GRAPPLE_STATE_FLY) {
		...
		speed = CTFApplyResistance(self->owner, CTF_GRAPPLE_PULL_SPEED);
...
//	VectorSet(offset, 24, 16, ent->viewheight-8+2);
	...
	speed = CTFApplyResistance(ent, CTF_GRAPPLE_SPEED);
...
int CTFApplyResistance(edict_t *ent, int speed)
{
...
qboolean CTFHasStrength(edict_t *ent)
{
	static gitem_t *tech = NULL;
...
void CTFApplyStrength(edict_t *ent)
{
	static gitem_t *tech = NULL;
...
qboolean CTFHasHaste(edict_t *ent)
{
	static gitem_t *tech = NULL;
...
pmenu_t creditsmenu[] = {
	...
	{ "*Double Domination",				PMENU_ALIGN_CENTER, NULL, NULL },
