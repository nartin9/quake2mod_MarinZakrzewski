...
void blaster_touch (edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	{
		if(CTFHasRegeneration(self->owner))
			return;
		gi.WriteByte (svc_temp_entity);
...
static void proximity_think(edict_t *self)
{
	...
	while (target)
	{
		if(((strcmp(target->classname, "player") == 0 && target->client->resp.ctf_team != self->owner->client->resp.ctf_team)|| (target->svflags & SVF_MONSTER)) && target->health > 0 && visible(self,target) && target != self->owner)
		{
...
static void health_think(edict_t *self)
{
	...
	while (target)
	{
		if((strcmp(target->classname, "player") == 0 && target->client->resp.ctf_team == self->owner->client->resp.ctf_team) && target->health > 0 && visible(self,target))
		{
			player = target;
		}
		target = findradius(target, self->s.origin, 100);
