...
void Weapon_Blaster_Fire (edict_t *ent)
{
	...
	if(CTFHasRegeneration(ent))
	{
		...
		VectorSet(diffVecs, 0, 6, 0);
		VectorAdd(diffVecs, vec3_origin, diffVecs);
		Blaster_Fire (ent, diffVecs, damage, false, EF_BLASTER);

		VectorSet(diffVecs, 0, -6, 0);
		VectorAdd(diffVecs, vec3_origin, diffVecs);
		Blaster_Fire (ent, diffVecs, damage, false, EF_BLASTER);

		VectorSet(diffVecs, 6, 0, 6);
		VectorAdd(diffVecs, vec3_origin, diffVecs);
		Blaster_Fire (ent, diffVecs, damage, false, EF_BLASTER);
...
void Weapon_HyperBlaster (edict_t *ent)
{
	...
	Weapon_Generic (ent, 3, 20, 45, 49, pause_frames, fire_frames, Weapon_HyperBlaster_Fire);
}
...
void Weapon_SuperShotgun (edict_t *ent)
{
	...
	Weapon_Generic(ent, 6, 17, 57, 61, pause_frames, fire_frames, Chaingun_Fire);
}
